# coba gunakan loofp for untuk mengiterasi item dalam list
list_angka = [1,2,3,4,5]
print(f"{list_angka[0]}")
indeks = 0
for angka in list_angka:
    print(f"dalam list angka terdapat angka = {angka} pada indeks ke {indeks}")
    indeks=indeks+1

# membuat seuah urutan angka mengguakan perulangan for dan funsi range()
for x in range(1,5):
    print(f"angka pertama yang dihasilkan leh fungsi range  ialah = {x}")
    
# hal diatas juga dapat dilakukan dengan menggunakan while sperti 
count = 0 
while count < 5:
    print(count)
    count+=1
